...
const HotOffersScreen = ({ navigation }) => {
    ...
    async function Get_Items() {
        ...
        //Filtiring for InOffer
        let temp_All_Car_Parts = [];
        await firestore().collection("CarStuff").get().then((Car_Stuff) => {
            for (let i = 0; i < Car_Stuff.docs.length; i++) {
                if (Car_Stuff.docs[i].data().InOffer == "true") {
                    temp_All_Car_Parts.push(Car_Stuff.docs[i].data());
                }
            }
        })
        //Filtring For Brand and Model
        for (let i = 0; i < temp.length; i++) {
            for (let j = 0; j < temp_All_Car_Parts.length; j++) {
                if (temp_All_Car_Parts[j].Car_Brand == temp[i].Brand && temp_All_Car_Parts[j].Car_Model == temp[i].Model)
                    temp_Car_Parts_List.push(temp_All_Car_Parts[j])
            }
        }
        setShow_Items(temp_Car_Parts_List);
        setItems(temp_Car_Parts_List);
        setloading(false);
    }
...
export default HotOffersScreen;
